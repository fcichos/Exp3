...
The path length difference given above is only approximately correct. The exact calculation would involve the geometry of the problem. For two sources separated by a distance $d$ at positions $z_1 = -d/2$ and $z_2 = d/2$, and a point P at distance $L$ from the center with coordinate $y_P$, the path lengths are $r_1 = \sqrt{L^2 + (y_P + d/2)^2}$ and $r_2 = \sqrt{L^2 + (y_P - d/2)^2}$, giving $\Delta s = r_2 - r_1$. The approximation $\Delta s \approx d\sin(\theta)$ is valid when the screen distance $L$ is much larger than both the source separation $d$ and the position on the screen $y_P$ (the far-field or **Fraunhofer limit**). Note that when observing the pattern on the screen with the help of a lens that is placed at the focal distance from the screen, the two paths would correspond to parallel rays and the path length difference assumed above would be exact.
:::
